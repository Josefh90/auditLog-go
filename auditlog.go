...
func DeleteCallback(db *gorm.DB) {
	...
	// Serialize the deleted record (db.Statement.Dest) to JSON
	oldData, err := json.Marshal(db.Statement.Dest)
	if err != nil {
		return // Silently fail on serialization errors
	}
	...
	log := AuditLog{
		TableName: db.Statement.Schema.Table,  // Name of the table where the operation occurred
		Action:    "DELETE",                   // Action type (DELETE in this case)
		OldData:   datatypes.JSON(oldData),   // Serialized deleted record
		CreatedAt: time.Now(),                // Timestamp of the operation
	}
